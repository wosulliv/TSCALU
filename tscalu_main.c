...
int main(int argc, char *argv[])
{
	...
	const int b = block_height/2; 			/* The number of rows that would be passed along from each round of tournament pivoting.
	...
	for (int d = 0; d < log2(lastpower); d++) 
	{
		...
		/* Performing communication between senders and receivers in a specified layer. */
				...
				combine_winners(A, W, W_buffer, block_height, n_dim); 				/* Form the new sub-matrix from the winners of the round. */
				mkl_free(W);
			}
	...
	if(rank == 0)
	{
		...
		pivot_contestants(A, block_height,n_dim, local_index, local_k, local_i);
		...
		perform_pivoting(mat, global_index, global_k, global_i, local_k, local_i, m_dim, n_dim, log2size, size);
		...
		double mat_inf_norm = infinity_norm(mat, m_dim, n_dim);
		double LU_inf_norm = infinity_norm(LU, m_dim, n_dim);
		const double growth_factor = LU_inf_norm/mat_inf_norm;		
		const double backwards_stability = (mat_inf_norm - LU_inf_norm)/original_mat_inf_norm;
		//fprintf(stdout, "Runtime: %f\n", runtime);
		//printf("Growth Factor: %f \n", growth_factor);
	
		/* NOTE: Could expand results to incldue the backwards stability (mat_inf_norm - LU_inf_norm) / original_mat_inf_norm */
