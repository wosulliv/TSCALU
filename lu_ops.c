...
void perform_pivoting(double * const mat, int * const global_index, int * const global_k, int * const global_i, int * const local_k, int * const local_i, const int m_dim, const int n_dim, const int log2size, const int size)
{
	...
	for(int i = 0; i < m_dim; i++){for(int j = 0; j < n_dim; j++){mat_tmp[i*n_dim+j] = mat[i*n_dim+j];}}	
	...
	//for(int d = 0; d < log2size; d++){for(int r = d*size*n_dim; r < (d+1)*size*n_dim; r++){}} /* Could equally use this condition for the values of r. */
	...
	for(int i = 0; i < m_dim; i++)
	{
		for(int kay = 0; kay < n_dim; kay++)
                {
                	mat[i*n_dim+kay] = mat_tmp[global_index[i]*n_dim+kay];
		}
	}
